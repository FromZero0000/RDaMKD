...
from mdistiller.engine.trainer import MuLoggerTrainer
...
def main(cfg, resume, opts):
    ...
    if cfg.LOG.WANDB:
        try:
            ...
            wandb.init(project=cfg.EXPERIMENT.PROJECT, name=experiment_name,
                       tags=tags, dir='wandb', mode="offline")
        except Exception:
    ...
    # 固定类别数，后面 teacher / student 都会用到
    num_classes = cfg.DATASET.NUM_CLASSES

    # 获取训练与验证 dataloader
    dataset_out = get_dataset(cfg)
    if len(dataset_out) == 4:
        train_loader, val_loader, num_data, _ = dataset_out
    else:
        train_loader, val_loader, num_data = dataset_out
    ...
    # train
    if cfg.DISTILLER.TYPE in ["MANIFOLD_KD", "ManifoldAlignKD_CKA"]:
        trainer = MuLoggerTrainer(
