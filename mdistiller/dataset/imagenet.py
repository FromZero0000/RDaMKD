...
from ._multi_view import MultiViewTransform, rand_augment


data_folder = os.environ.get(
    "IMAGENET_ROOT",
    os.path.abspath(os.path.join(os.path.dirname(__file__), "../../data/imagenet")),
)
...
def get_imagenet_train_transform(mean, std, input_size=224, strong=False):
    ...
    strong_transform = transforms.Compose(
        [
            ...
            transforms.RandomErasing(
                p=1.0, scale=(0.02, 0.25), ratio=(1.0, 1.0), value=0.5
            ),
            normalize,
        ]
    )
    return MultiViewTransform(weak_transform, strong_transform)
...
def get_imagenet_dataloaders(
    ...
    train_transform = get_imagenet_train_transform(mean, std, strong=strong)
    ...
    test_loader = get_imagenet_val_loader(
        val_batch_size, mean, std, num_workers
    )
